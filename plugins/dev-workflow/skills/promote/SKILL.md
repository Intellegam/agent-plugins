...
- An explicit request to promote, ship, or release authorizes preparing and merging the promotion once its gates pass. A request only to assess or prepare a PR does not authorize merging. Present the final gate before executing, but do not ask again for authorization already given. Honor an explicit user request for a separate confirmation checkpoint.
- Never infer branch names, merge strategy, release semantics, or deployment health.
- Stop when policy and live configuration materially disagree. Report both sources instead of choosing one silently. A documented check missing from branch protection is an enforcement gap, not a waiver of the check or an automatic blocker; evaluate it explicitly as described in Section 6.
...
- **Promote/ship/release**: assess, prepare the PR, present the final gate, and merge once ready without another confirmation, unless the user or repository requires a separate checkpoint that the user has not explicitly waived.
- **Merge this exact ready PR**: revalidate and present the gate; execute the explicitly authorized merge once ready.
- **Merge this promotion when ready / do not ask again**: assess, prepare the PR, present the gate, and merge once ready without another confirmation. This authorizes a direct merge within the current promotion scope, not GitHub auto-merge, a queue, or bypassing failed checks. An explicit current-session instruction to proceed without another confirmation overrides an earlier user preference for that checkpoint.
...
Verify the union of required gates from repository policy and live GitHub rules; GitHub mergeability alone is not sufficient: checks must succeed for the recorded gate SHA, required reviews must be satisfied, and any required conversation-resolution gate must be clear. Do not invent an external reviewer requirement when neither policy nor GitHub requires one. Admin status or bypass capability alone is not a blocker and does not waive failing or pending gates. When a documented check is not server-enforced, require its successful result explicitly; report the enforcement gap and verify it again immediately before merging. Do not change repository settings or use an admin override to bypass an unsatisfied gate. If policy explicitly requires non-bypass enforcement itself, treat its absence as a policy conflict.

Include the relevant tag refs, GitHub Release baseline, and pending/running release-workflow state in the gate snapshot. Request separate merge approval only when the request was limited to assessment/PR preparation, or the user explicitly retained that checkpoint. A promotion request supplies bounded authorization for this promotion once ready. Admin status alone is not merge authorization. Preserve the scope of any authorization already given. A promotion request or explicit instruction to merge when ready supplies merge authorization; it does not skip the independent review or its `PROCEED` decision.

Immediately before merging, refetch the PR head, base head, checks, reviews, mergeability, auto-merge/queue state, policy, release configuration, release baseline, and release-workflow state. If any changed, recompute and present the complete gate. Approval tied to an exact snapshot requires fresh approval; bounded authorization to merge when ready remains valid only while the promotion scope is unchanged and all gates pass again. Stop for new blockers or changes outside that authorization, including changed source/target heads or release policy; do not silently promote newly added commits. Pass the freshly verified PR head SHA as a server-side precondition and merge only with the documented method. GitHub atomically binds only that head SHA: applicable server enforcement protects its configured gates, while manually verified gates, base state, and release evidence retain refetch-and-verify races. Disclose those residual races in the merge gate, return `HOLD` when policy requires atomic binding that GitHub cannot provide, and otherwise verify after merging that the resulting target history is rooted at the final refetched base. Accept a changed release baseline only when Section 7 proves it was caused by this merge; treat any other change as incomplete verification. Auto-merge and merge queues are out of scope; a repository policy that merely permits auto-merge does not require this skill to use it. If auto-merge is already armed or the PR is queued, report the unowned pending mutation and stop rather than changing it or proceeding.
